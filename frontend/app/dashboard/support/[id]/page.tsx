...
import { fetchMyTicket, replyTicket } from "@/lib/services/account"
...
export default function TicketDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const load = async () => {
    setLoading(true)
    try {
      const t = await fetchMyTicket(id)
      setTicket(t)
    } catch (err) {
        ...
        <div className="mt-3 flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              #{ticket.ticketNumber ?? ticket.id.slice(0, 8)} · {ticket.priority} ·{" "}
      ...
      <div className="space-y-3">
        ...
        {messages.map((m) => (
          <div
            key={m.id}
            className={cn(
              "max-w-3xl rounded-xl border border-border p-4",
              m.sender?.role === "CUSTOMER" ? "bg-card" : "bg-muted/50",
            )}
          >
